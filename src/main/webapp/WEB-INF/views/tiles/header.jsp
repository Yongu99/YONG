...
$(document).ready(function() {
    ...
    eventSource.onmessage = function(event) {
        ...
        if (data.alarmCategory != 'chat'&&data.alarmCategory != 'chatOp') {
        	...
        	let message = content.msgContent;

            // HTML 태그 제거 (줄바꿈을 유지하면서 <br>과 <div> 같은 태그 제거)
            message = message.replace(/<\/?[^>]+(>|$)/g, "");

            // 메시지를 두 줄까지만 표시하고, 더 길면 '...' 추가
            const maxLength = 50; // 원하는 최대 글자 수
            ...
            $("#messageContent").text(message);  // 채팅 내용 설정
